...
class Attention(nn.Module):
    ...
    def __init__(self, dim, qkv_bias=False,
        ...
        super().__init__()
        self.num_heads = int(dim/2)  # 计算注意力头的数量
        num_heads = int(dim/2)       # 同样计算注意力头的数量
        head_dim = dim // num_heads  # 计算每个注意力头的维度
        self.scale = qk_scale or head_dim**-0.5  # 如果未指定缩放因子，使用默认值
...
class Generic_UNet(SegmentationNetwork):
    ...
    MAX_NUMPOOL_3D = 999
    MAX_NUM_FILTERS_3D = 512 # 修改这里可以增加网络的深度，在魔改nnunet论文里有巨大提升
